...
impl ServiceBackend for OpnsenseBackend {
    ...
    fn runtimes(&self) -> Vec<Runtime> { vec![Runtime::Vm] }
        ...
        -> BoxFuture<'a, Result<BackupArtifact, ServiceError>>
    {
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("opnsense.backup")) })
    }

    fn restore<'a>(&'a self, _ep: &'a Endpoint, _from: &'a BackupArtifact)
        -> BoxFuture<'a, Result<(), ServiceError>>
    {
        Box::pin(async move { Err(ServiceError::unimplemented("opnsense.restore")) })
    }
